...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    //  this.setState({ all : all[this.state.account]})
      ...
      // this.setState({ occ })
      const name1 = await dstorage.methods.name1(this.state.account).call()
      this.setState({
        name1: [...this.state.all, name1]
      })
  ...
  //  console.log("Submitting file to IPFS...")
    ...
    // Add file to the IPFS
      ...
      this.state.dstorage.methods.login(password,name1).send({ from: this.state.account }).on('transactionHash', () => {
